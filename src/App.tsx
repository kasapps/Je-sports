    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        if (userDoc.exists()) {
          ...
          // Create default user if not exists (first login)
          const newUser: User = {
            id: firebaseUser.uid,
            username: firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'Usuario',
            role: firebaseUser.email === 'farmacia.soluciones.5@gmail.com' ? 'admin' : 'seller'
          };
  ...
  return (
    <ErrorBoundary>
      ...
      <motion.aside 
        ...
        className="bg-[#151619] text-white flex flex-col shadow-2xl z-20"
      >
